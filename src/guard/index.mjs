...
export async function attachGuard({
    ...
    if (!dnsCache.has(hostname)) {
      dnsCache.set(
        ...
        Promise.resolve()
          ...
          .then((addresses) => checkResolvedAddresses(addresses).ok)
          .catch(() => false),
      );
    }
  ...
  await page.route("**/*", async (route) => {
    ...
    // checkUrl hanya memeriksa bentuk hostname. Sebelum asset benar-benar
    // diteruskan, resolve DNS-nya juga supaya domain publik yang mengarah ke
    // jaringan privat tidak dapat melewati guard.
    if (decision.allow && CHECKED_TYPES.includes(type)) {
      if (!(await publicHost(url))) {
        decision = block(
          "asset-address-not-public",
          "Alamat asset tidak menghasilkan IP publik yang aman.",
        );
      }
    }
