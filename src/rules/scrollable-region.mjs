  ...
  return page.evaluate(
    ({ selectors, maxHeadingLength }) => {
      ...
      return selectors.map((selector) => {
        ...
        const patches = [];
        // tabindex="-1" masih tidak dapat dicapai dengan tombol Tab. Axe dapat
        // melaporkan keadaan ini, jadi nilai negatif harus diperbaiki juga.
        if (!element.hasAttribute("tabindex") || element.tabIndex < 0) {
          patches.push({
            attribute: "tabindex",
            value: "0",
            replaceExisting: element.hasAttribute("tabindex") && element.tabIndex < 0,
          });
        }
