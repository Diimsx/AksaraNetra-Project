...
test("checks asset DNS before continuing and caches a public host", async () => {
  let handler;
  let resolveCalls = 0;
  const page = {
  ...
  const makeRoute = () => ({
    ...
    abort: async () => outcomes.push("blocked"),
  });

  await handler(makeRoute());
  await handler(makeRoute());

  assert.deepEqual(outcomes, ["continued", "continued"]);
  assert.equal(resolveCalls, 1);
});

test("blocks an asset whose DNS resolves to a private address", async () => {
  let handler;
  let outcome = "";
  const page = {
    async route(_pattern, callback) {
