...
test("reader settings work without adding script", () => {
  const html = renderReaderDocument({
    title: "Halaman contoh",
    sourceUrl: SOURCE,
    disclaimer: DISCLAIMER,
  ...
  assert.match(html, /Pengaturan tampilan bacaan/);
  assert.match(html, /id="teks-besar"/);
  assert.match(html, /id="tema-kontras"/);
  assert.doesNotMatch(html, /<script\b/i);
});

test("renders navigation, quotes, figures and complex tables semantically", () => {
  const html = renderReaderDocument({
    title: "Data daerah",
    sourceUrl: SOURCE,
    disclaimer: DISCLAIMER,
    blocks: [
      {
        ...
        links: [{ text: "Beranda", href: "/" }],
      },
      { type: "quote", text: "Pelayanan untuk semua." },
      {
        type: "figure",
        src: "/foto.jpg",
        alt: "Petugas melayani warga",
        caption: "Pelayanan publik",
      },
      {
        type: "table",
        caption: "Jumlah layanan",
        rows: [
          [{ text: "Kabupaten", header: true, scope: "col", colspan: 1, rowspan: 1 }],
          [{ text: "Gowa", header: false, colspan: 1, rowspan: 1 }],
        ],
      },
    ],
  });

  assert.match(html, /<details class="navigasi-sumber">/);
  assert.match(html, /<blockquote>Pelayanan untuk semua\.<\/blockquote>/);
  assert.match(html, /<figure><img[^>]+><figcaption>Pelayanan publik<\/figcaption>/);
  assert.match(html, /role="region" aria-label="Jumlah layanan" tabindex="0"/);
  assert.match(html, /<th scope="col">Kabupaten<\/th>/);
});

test("renders safe links inside paragraph text", () => {
  const html = renderReaderDocument({
    title: "Halaman contoh",
    sourceUrl: SOURCE,
    disclaimer: DISCLAIMER,
    blocks: [
      {
        ...
        parts: [
          { type: "text", text: "Baca " },
          { type: "link", text: "berita lengkap", href: "/berita/1" },
          { type: "text", text: "." },
        ],
      },
    ],
  ...
  assert.match(html, /Baca <a href="https:\/\/sulselprov\.go\.id\/berita\/1"/);
});
