...
async function setup({ rateLimit = 10 } = {}) {
  ...
  const store = new JobStore({ databasePath: config.databasePath });
  const worker = {
    kicks: 0,
    kick() { this.kicks += 1; },
    cancel(id) { return store.requestCancel(id); },
  };
  ...
  return { root, config, store, worker, base, close };
}
...
test("returns expiring reader and PDF links only after completion", async () => {
  const item = await setup();
  const created = await createJob(item);
  const job = item.store.claimNext();
  ...
  item.store.complete(job.id, { snapshot: { summary: { beforeTotal: 2, afterTotal: 0 } } });
...
test("lets the user explicitly reuse a stored result", async () => {
  ...
  const original = item.store.claimNext();
  ...
  item.store.complete(original.id, { snapshot: { cached: true } });
  ...
  assert.equal(item.worker.kicks, 1);
  assert.equal(item.store.getJob(reused.body.jobId).result.snapshot.cached, true);
  await item.close();
});
